...
function aboutMeHandler(req, res) {
  res.status(200).send('My Name is Kevin and im better than you');
}
function favFoodHandler(req, res) {
  res.status(200).send('Mac N Cheese');
}
...
const count = (target, input) => {
  return input.reduce((ASF, value) => {
    let rowcount = value.reduce((ASF2, value2) => {
      if (value2 === target) {
        return ASF2 + 1;
      }
      return ASF2;
...
let findShortest = (data) => {
  let shortest = data.reduce((acc, char) => {
    let sortArray = [char.height, acc];
    sortArray.sort((a,b) => a -b);
    acc = sortArray[0];
    return acc;
  }, 1000);
  return data.filter(char => char.height === shortest).reduce((acc, value) => {
    acc = value.name;
    return acc;
  }, '');
};
